...
SHALL be the folder name, and the new project SHALL be persisted.

#### Scenario: Add a valid folder

- **WHEN** the user adds an existing folder that is not already registered
- **THEN** a project is created, named after the folder, and written to `config.json`
...
The system SHALL remove a project from the registry and persist the change without
touching files on disk.

#### Scenario: Remove an entry

- **WHEN** the user removes a project
- **THEN** the entry disappears from the registry and `config.json`, and the folder on disk is left intact
